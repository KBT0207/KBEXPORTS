    ...
    def apply_regex(self):
        ...
        def net_weight_column_create_condition(row):
            try:
                ...
                print(f"Error calculating net weight for row: {row}. Error: {str(e)}")
                return 0
    ...
    def generate_product_report(self,file):
        ...
        with pd.ExcelWriter(file, engine='xlsxwriter') as writer:
            for product, df in transformed_product_list.items():
                ...
                #     sum_row = f"{{=sum({number_to_excel_column(row+2)}{row+1}:{number_to_excel_column(row+2)}{num_rows})}}"

                #     worksheet.write_formula(f"{number_to_excel_column(row+2)}{num_cols+1}",sum_row, border_format )
